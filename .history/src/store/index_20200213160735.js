...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    joinUserEvent(state, payload) {
      const id = payload.id;
      if (state.user.joinedEvents.findIndex(event => event.id === id) >= 0) {
        return;
      }
  ...
  actions: {
    ...
    joinUserEvent({ commit, getters }, payload) {
    ...
    getUser({ commit, getters }) {
      firebase
        .database()
        .ref("/users/" + getters.user.id + "/events")
        .once("value")
        .then(data => {
          const dataP = data.val();
          let change = {};
          let joinedEvents = [];
          for (let key in dataP) {
            joinedEvents.push(dataP[key]);
            change[dataP[key]] = key;
          }
          const updateUser = {
            id: getters.user.id,
